  ...
  return (
    <div>
        <div className='max-w-7xl mx-auto px-4 py-8'>
            ...
            text-transparent max-sm:hidden'>
            Welcome, Maysara Mohamed
            </p>
            <div className='flex gap-4'>
                ...
                border-slate-300 group hover:border-indigo-500 hover:shadow-lg 
                    ...
                    <PlusIcon className='size-11 transition-all duration-300 p-2.5 bg-gradient-to-br
                    from-indigo-300 to-indigo-600 text-white rounded-full'/>
                    <p className='text-sm group-hover:text-indigo-600 transition-all
                    duration-300'>Create Resume</p>
                </button>
                <button className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed
                border-slate-300 group hover:border-purple-500 hover:shadow-lg 
                    transition-all duration-300 cursor-pointer'>
                    <UploadCloudIcon className='size-11 transition-all duration-300 p-2.5 bg-gradient-to-br
                    from-purple-300 to-purple-600 text-white rounded-full'/>
                    <p className='text-sm group-hover:text-indigo-600 transition-all
            ...
            </div>
            {
                showCreateResume && (
                    <form onSubmit={createResume} onClick={()=> setShowCreateResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
                        <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md p-6 rounded-lg w-full max-w-sm '>
                            <h2 className='text-xl font-bold mb-4'>Create a Resume</h2>
                            <input
                            type='text'
                            placeholder='Enter resume title'
                            className='w-full px-4 py-2 mb-4 border rounded focus:border-green-600 focus:ring-green-600 outline-none'
                            required
                            />
                            <button
